...
def solution(park, routes):
    row_size = len(park)
    col_size = len(park[0])  # 정사각형이 아니다. 세로·가로를 따로 잡는다
    ...
    # 시작점 'S' 찾기. 공원에 단 하나뿐이라 찾는 즉시 멈춘다
    ...
    for route in routes:
        ...
        for _ in range(number):
            ...
            # 범위 검사를 격자 접근보다 먼저 해야 한다.
            # 파이썬은 음수 인덱스가 뒤에서부터 읽히므로 에러조차 나지 않고 조용히 틀린다
...
test_cases = [
    ...
    # 경계 ① 첫 명령부터 무시된다
    ((["SOO", "OOO", "OOO"], ["N 1", "E 1"]), [0, 1]),
    # 경계 ② 모든 명령이 무시되어 시작점 그대로
    ((["OOO", "OSO", "OOO"], ["N 2", "S 2", "W 2", "E 2"]), [1, 1]),
    # 경계 ③ 세로·가로가 다른 공원에서 벽에 딱 붙어 끝난다
    ((["OOOOO", "OOOOS"], ["S 1", "W 4"]), [1, 0]),
]
